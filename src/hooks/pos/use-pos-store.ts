...
interface POSState {
  ...
  // UI State
  currentView: 'catalog' | 'cart' | 'payment' | 'receipt'
  isLoading: boolean
  ...
  setCurrentView: (view: 'catalog' | 'cart' | 'payment' | 'receipt') => void
...
export const usePOSStore = create<POSState>((set, get) => ({
  ...
  removeFromCart: (productId) => {
    const state = get()
    set({ cartItems: state.cartItems.filter(item => item.product.id !== productId) })
    get().calculateTotals()
  },
  ...
  getOrderData: () => {
    const state = get()
    return {
      custumer_id: state.selectedCustomer?.id || null,
      pet_id: state.selectedPet?.id || null,
