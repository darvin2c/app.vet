...
export function useProductCategoryCreateBulk() {
  ...
  return useMutation({
    mutationFn: async (
      ...
      const productCategoryData: TablesInsert<'product_categories'>[] =
        data.map((item) => ({
          ...item,
      ...
      if (error) {
        throw new Error(
          `Error al crear categorías de productos: ${error.message}`
        )
      }
    ...
    onSuccess: (productCategories) => {
      ...
      toast.success(
        `Se crearon ${productCategories.length} categorías de productos exitosamente`
      )
    },
    onError: (error) => {
