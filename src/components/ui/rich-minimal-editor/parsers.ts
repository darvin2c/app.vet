export function toWhatsAppText(html: string): string {
  ...
  const renderNode = (node: Node): string => {
    ...
    if (tag === 'UL') {
      ...
      return items.map((li) => `- ${sanitizeText(Array.from(li.childNodes).map(renderNode).join(''))}`).join('\n') + '\n\n'
    }
    if (tag === 'OL') {
      const items = Array.from(el.children).filter((c) => c.tagName === 'LI') as HTMLElement[]
      return (
        items
          .map((li, i) => `${i + 1}. ${sanitizeText(Array.from(li.childNodes).map(renderNode).join(''))}`)
          .join('\n') + '\n\n'
      )
    }
    if (tag === 'BLOCKQUOTE') {
...
export function toHtmlEmail(html: string): string {
  ...
  const removeUnsafeAttrs = (el: Element) => {
    ...
    for (const a of attrs) {
      ...
      if (name === 'class' || name.startsWith('on') || name.startsWith('data-')) el.removeAttribute(a.name)
    }
  }
  ...
  for (const el of elements) {
    ...
    if (tag === 'P') el.setAttribute('style', 'margin:0 0 8px;line-height:1.6;font-size:14px')
    else if (tag === 'UL' || tag === 'OL') el.setAttribute('style', 'margin:0 0 8px;padding-left:20px')
    else if (tag === 'LI') el.setAttribute('style', 'margin:0 0 4px;line-height:1.6')
    else if (tag === 'A') {
